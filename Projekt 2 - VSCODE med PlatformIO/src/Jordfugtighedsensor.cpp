...
void Jordfugtighedsensor::ADC_init() {
	ADMUX = (1 << REFS0);  // S�tter reference til AVcc (5V)
	ADCSRA = (1 << ADEN) | (1 << ADPS2) | (1 << ADPS1) | (1 << ADPS0);  // Aktiverer ADC og s�tter prescaler
}
...
double Jordfugtighedsensor::ADC_readAsPercentage(){  // Retunere Humidity som procent
	...
	while (ADCSRA & (1 << ADSC));  // Venter p�, at konvertering er f�rdig
	
	double moisturePercentage = ((1023 - ADC) * 100) / (1023 - 300); // ADC registeret indeholder v�rdien for det digitale signal 
